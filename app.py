import os

from cs50 import SQL
from flask import Flask, redirect, render_template, request, session
from flask_session import Session
from werkzeug.security import check_password_hash, generate_password_hash

from helpers import apology, login_required, lookup, usd

app = Flask(__name__)

app.jinja_env.filters["usd"] = usd

# Configure session to use filesystem (instead of signed cookies)
app.config["SESSION_PERMANENT"] = False
app.config["SESSION_TYPE"] = "filesystem"
Session(app)

# Configure CS50 Library to use SQLite database
db = SQL("sqlite:///finance.db")


@app.after_request
def after_request(response):
    """Ensure responses aren't cached"""
    response.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
    response.headers["Expires"] = 0
    response.headers["Pragma"] = "no-cache"
    return response


@app.route("/")
def index():
    """Show portfolio of stocks"""
    if session:
        user = db.execute(
            "SELECT username, cash FROM users WHERE id = ?", session["user_id"]
        )

        stock_owned = db.execute(
            "SELECT symbol, shares FROM portfolio WHERE user_id = ?", session["user_id"]
        )

        total_cash = int(user[0]["cash"])
        prices = []
        total_value = []
        for row in stock_owned:
            price = lookup(row["symbol"])
            prices.append(usd(price["price"]))
            total_cash += price["price"] * row["shares"]
            total_value.append(usd(price["price"] * row["shares"]))

        user[0]["cash"] = usd(user[0]["cash"])
        total_cash = usd(total_cash)
        return render_template(
            "index.html",
            name=user[0]["username"],
            total_cash=total_cash,
            cash_available=user[0]["cash"],
            stock_owned=stock_owned,
            prices=prices,
            total_value=total_value,
        )

    return render_template("index.html")


@app.route("/login", methods=["GET", "POST"])
def login():
    """Log user in"""

    # Forget any user_id
    session.clear()

    # User reached route via POST (as by submitting a form via POST)
    if request.method == "POST":
        # Ensure username was submitted
        if not request.form.get("username"):
            return apology("must provide username")

        # Ensure password was submitted
        elif not request.form.get("password"):
            return apology("must provide password")

        # Query database for username
        rows = db.execute(
            "SELECT * FROM users WHERE username = ?", request.form.get("username")
        )

        # Ensure username exists and password is correct
        if len(rows) != 1 or not check_password_hash(
            rows[0]["hash"], request.form.get("password")
        ):
            return apology("invalid username and/or password")

        # Remember which user has logged in
        session["user_id"] = rows[0]["id"]

        # Redirect user to home page
        return redirect("/")

    # User reached route via GET (as by clicking a link or via redirect)
    else:
        return render_template("login.html")


@app.route("/logout")
def logout():
    """Log user out"""

    # Forget any user_id
    session.clear()

    # Redirect user to login form
    return redirect("/")


@app.route("/register", methods=["GET", "POST"])
def register():
    """Register user"""

    if request.method == "POST":
        if not request.form.get("username"):
            return apology("must provide username")

        if not request.form.get("password"):
            return apology("must provide password")

        if request.form.get("password") != request.form.get("confirmation"):
            return apology("password and confirmation must be the same")

        user_exists = db.execute(
            "SELECT * FROM users WHERE username = ?", request.form.get("username")
        )
        if user_exists:
            return apology("Username already exists, pick another")

        username = request.form.get("username")
        hpassword = generate_password_hash(request.form.get("password"))
        db.execute(
            "INSERT INTO users (username, hash) VALUES (?,?)",
            username,
            hpassword,
        )
        return redirect("/")

    return render_template("register.html")


@app.route("/quote", methods=["GET", "POST"])
@login_required
def quote():
    """Get stock quote."""
    if request.method == "POST":
        if not request.form.get("symbol"):
            return apology("Insert stock symbol")

        symbol = request.form.get("symbol")

        quote = lookup(symbol)
        if quote is None:
            return apology("Invalid stock symbol")

        return render_template("quoted.html", quote=quote)

    return render_template("quote.html")


@app.route("/buy", methods=["GET", "POST"])
@login_required
def buy():
    """Buy shares of stock"""
    if request.method == "POST":
        if not request.form.get("symbol"):
            return apology("Please insert a stock symbol")

        quote = lookup(request.form.get("symbol"))
        if quote is None:
            return apology("Please insert a valid stock symbol")

        if not request.form.get("shares") or float(request.form.get("shares")) < 1:
            return apology("Please insert a positive number")

        cash = get_cash()
        if cash < quote["price"] * float(request.form.get("shares")):
            return apology("Not enough cash to complete transaction")

        # Store transaction details
        db.execute(
            "INSERT INTO transactions (user_id, symbol, price, shares, type) VALUES (?, ?, ?, ?, ?)",
            session["user_id"],
            quote["symbol"],
            quote["price"],
            request.form.get("shares"),
            "buy",
        )

        # Deduct price from user's cash
        remaining_cash = cash - quote["price"] * int(request.form.get("shares"))
        db.execute(
            "UPDATE users SET cash = ? WHERE id = ?", remaining_cash, session["user_id"]
        )

        # Update user's portfolio
        existing_stock = db.execute(
            "SELECT shares FROM portfolio WHERE user_id = ? AND symbol = ?",
            session["user_id"],
            quote["symbol"],
        )
        if not existing_stock:
            db.execute(
                "INSERT INTO portfolio (user_id, symbol, shares) VALUES (?, ?, ?)",
                session["user_id"],
                quote["symbol"],
                request.form.get("shares"),
            )
        else:
            bought = existing_stock[0]["shares"] + int(request.form.get("shares"))
            db.execute(
                "UPDATE portfolio SET shares = ? WHERE symbol = ?",
                bought,
                quote["symbol"],
            )

        return redirect("/")

    return render_template("buy.html")


@app.route("/sell", methods=["GET", "POST"])
@login_required
def sell():
    """Sell shares of stock"""
    stock_owned = db.execute(
        "SELECT symbol, shares FROM portfolio WHERE user_id = ?", session["user_id"]
    )

    if request.method == "POST":
        stock = request.form.get("symbol")
        shares_owned = db.execute(
            "SELECT symbol, shares FROM portfolio WHERE ?", session["user_id"]
        )

        if not request.form.get("shares") or int(request.form.get("shares")) < 1:
            return apology("Input valid amount of shares")

        stock_exists = any(stock == s["symbol"] for s in shares_owned)
        if not stock_exists:
            return apology("You do not own this stock")

        shares_to_sell = int(request.form.get("shares"))
        for s in shares_owned:
            if s["symbol"] == stock:
                if shares_to_sell > s["shares"]:
                    return apology("You do not own this many shares of this stock")
                break

        # Add sell_balance to user's available cash
        stock_info = lookup(stock)
        profit = stock_info["price"] * shares_to_sell
        old_cash = db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"])
        new_balance = old_cash[0]["cash"] + profit
        db.execute(
            "UPDATE users SET cash = ? WHERE id = ?", new_balance, session["user_id"]
        )

        # Remove stock from user portfolio
        remove = db.execute(
            "SELECT shares FROM portfolio WHERE user_id = ? AND symbol = ?",
            session["user_id"],
            stock,
        )
        owned_amount = remove[0]["shares"]
        shares_new = owned_amount - shares_to_sell
        db.execute(
            "UPDATE portfolio SET shares = ? WHERE user_id = ? AND symbol = ?",
            shares_new,
            session["user_id"],
            stock,
        )

        # Store sell transaction
        db.execute(
            "INSERT INTO transactions (user_id, symbol, price, shares, type) VALUES (?, ?, ?, ?, ?)",
            session["user_id"],
            stock_info["symbol"],
            stock_info["price"],
            shares_to_sell,
            "sell",
        )

        return redirect("/")

    return render_template("sell.html", stock_owned=stock_owned)


@app.route("/history")
@login_required
def history():
    """Show history of transactions"""
    history = db.execute(
        "SELECT * FROM transactions WHERE user_id = ?", session["user_id"]
    )

    return render_template("history.html", history=history)


def get_cash():
    # Query database for current user's cash amount
    cash = db.execute("SELECT cash FROM users WHERE id = ?", session["user_id"])

    return cash[0]["cash"]
